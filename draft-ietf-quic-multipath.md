...
For any given path, connection ID rotation, NAT Rebinding or path migration,
...
However otherwise, as a general rule, when the multipath extension is supported,
implementations  should strongly prefer opening a new path instead of path migration 
...
observe a different 4-tuple. Though changes the 4-tuple of the path,
